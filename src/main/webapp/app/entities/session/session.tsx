...
import { getEntities, reset } from './session.reducer';
...
export const Session = () => {
  ...
  const totalItems = useAppSelector(state => state.session.totalItems);
  const links = useAppSelector(state => state.session.links);
  const entity = useAppSelector(state => state.session.entity);
  ...
  return (
    <div>
      ...
      <div className="table-responsive">
        <InfiniteScroll
          dataLength={sessionList ? sessionList.length : 0}
          next={handleLoadMore}
        ...
        >
          {sessionList && sessionList.length > 0 ? (
            <Table responsive>
              <thead>
                <tr>
                  ...
                  <th className="hand" onClick={sort('sessionNotes')}>
                    Session Notes <FontAwesomeIcon icon="sort" />
                  </th>
                  <th>
              ...
              <tbody>
                {sessionList.map((session, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{session.date ? <Link to={`/calendar-day/${session.date.id}`}>{session.date.id}</Link> : ''}</td>
            ...
            !loading && <div className="alert alert-warning">No Sessions found</div>
          )}
        </InfiniteScroll>
      </div>
    </div>
